...
def create_how_it_works_section() -> list:
    ...
    return [
        ...
        html.P(
            """
        The traceback matrix is filled, storing information about the operation 
        that resulted in choosing the optimal score at each possible pair of characters. 
        Based on the formula provided above, we have three possible outcomes. The matrix 
        is filled with points depending on whether a match/mismatch, 
        a gap in sequence X, or a gap in sequence Y was used."""
        ),
        html.H3("5. Tracing back the best alignment"),
        html.P(
            """
        The results are traced backward (starting from the end of the matrix) 
        - thanks to filling the matrix 'pointers_to_trace_optimal_alignment' 
        in the previous step, the algorithm knows the operation that led 
        to the alignment, and this is information about which field to move to. 
        The result of this process is a two sequences of characters, representing 
        the best alignment with each other."""
        ),
    ]
...
def create_explanation_layout() -> dbc.Container:
    ...
    return dbc.Container(
        [
            dbc.Row(
                dbc.Col(html.H2("About"), width={"size": 6, "offset": 3}),
                align="center",
            ),
            dbc.Row(
                dbc.Col(create_about_section(), width={"size": 8, "offset": 2}),
                align="center",
            ),
            dbc.Row(
                dbc.Col(html.H2("How it works"), width={"size": 6, "offset": 3}),
                align="center",
            ),
            dbc.Row(
                dbc.Col(create_how_it_works_section(), width={"size": 8, "offset": 2}),
                align="center",
            ),
        ],
        fluid=True,
